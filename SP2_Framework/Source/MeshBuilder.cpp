...
Mesh* MeshBuilder::GenerateLines(const std::string &meshName, Color color, float lengthX, float lengthY, float centerX, float centerY)
{
	// An array of 3 vectors which represents 3 vertices
	Vertex v;
	std::vector<Vertex> vertex_buffer_data;

	v.pos.Set(lengthX, lengthY, 5);
	v.color = color;
	vertex_buffer_data.push_back(v);
	
    v.pos.Set(centerX, centerY, 5);
	v.color = color;
	vertex_buffer_data.push_back(v);
