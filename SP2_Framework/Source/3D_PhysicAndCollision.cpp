...
void threeD_scene::Init()
{
    ...
    meshList[GEO_CUBE] = MeshBuilder::GenerateCube("cube", Color(0, 1.0f, 0), 1, 1, 1);
    ...
    translateX = translateY  = translateZ = tx = ty = rotate  = r = 0;
    scale = s = 1.f;
    velocity = 10.f;
}
...
*/
/******************************************************************************/
void threeD_scene::Update(double dt)
{
    ...
    if(Application::IsKeyPressed('D'))
    {
        ...
        translateZ += hb1.unitVecX.z * velocity * dt;
    }
    ...
    {
        translateY += hb1.unitVecZ.y * velocity * dt;
        translateX += hb1.unitVecZ.x * velocity * dt;
        translateZ += hb1.unitVecZ.z * velocity * dt;
    }
    ...
    if(Application::IsKeyPressed('S'))
    {
        translateY -= hb1.unitVecZ.y * velocity * dt;
        translateX -= hb1.unitVecZ.x * velocity * dt;
        translateX -= hb1.unitVecZ.z * velocity * dt;
    }
...
*/
/******************************************************************************/
void threeD_scene::RenderMesh(Mesh *mesh, bool enableLight)
{
...
*/
/******************************************************************************/
void threeD_scene::RenderText(Mesh* mesh, std::string text, Color color)
{
...
*/
/******************************************************************************/
void threeD_scene::RenderTextOnScreen(Mesh* mesh, std::string text, Color color, float size, float x, float y)
{
    if(!mesh || mesh->textureID <= 0) //Proper error check
		return;
...
void threeD_scene::Render()
{
    ...
    sceneStack.PushMatrix();
    sceneStack.Translate(translateX, translateY, translateZ);
    sceneStack.Rotate(r, 0, 0, 1);
    sceneStack.Scale(scale, scale, scale);
    RenderMesh(meshList[GEO_CUBE], false);

    hb1.Set(
        Translate * Rotate * Scale *Vector3(1 ,1, 1), 
        Translate * Rotate * Scale *Vector3(1 ,1, -1),
        Translate * Rotate * Scale *Vector3(-1 ,1, -1), 
        Translate * Rotate * Scale *Vector3(-1 ,1, 1),
        Translate * Rotate * Scale *Vector3(1 ,-1, 1), 
    ...
    // FPS
    RenderTextOnScreen(meshList[GEO_TEXT], Conv(1/deltaTime)+" fps", Color(1, 1, 1), 2, 2, 27);
    
    // coordinate
    RenderTextOnScreen(meshList[GEO_TEXT], "XYZ:"+ Conv(camera.position.x) + " / " + Conv(camera.position.y) +" / " + Conv(camera.position.z ), Color(1, 0,0), 2, 2, 25);

    // facing
    RenderTextOnScreen(meshList[GEO_TEXT], "Facing: " + directn(camera.NORTH(camera.north), camera.EAST(camera.east)) + " (" + Conv(camera.yaw) + "/" + Conv(camera.pitch) + ")", Color(1, 0, 0), 2, 2, 23);
}
